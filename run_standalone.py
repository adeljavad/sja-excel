#!/usr/bin/env python3
"""
Fixed standalone script for Smart Extractor
اسکریپت مستقل اصلاح شده برای سیستم استخراج هوشمند
"""
...
try:
    ...
    try:
        ...
        excel_processor_path = current_dir / 'processors' / 'excel_processor.py'
        spec = importlib.util.spec_from_file_location("excel_processor", excel_processor_path)
        excel_processor_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(excel_processor_module)
        ExcelProcessor = excel_processor_module.ExcelProcessor
        
        # Import FileHandler
        file_handler_path = current_dir / 'utils' / 'file_handler.py'
        ...
        print(f"❌ خطا در بارگذاری ماژول‌ها: {str(e)}")
        sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
نمونه استفاده:
  python run_standalone.py data.xlsx
  python run_standalone.py data.xlsx -o extracted_data
  python run_standalone.py data.xlsx --suffix "_processed"
        """
    )
